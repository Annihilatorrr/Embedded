	...
	if(READ_BIT(DMA1->ISR, DMA_ISR_TCIF3) == (DMA_ISR_TCIF3))
	  {
	    ...
	    LCD_CS_PORT->BSRR = (1<<LCD_CS_PIN);
	  }
}

static void ILI9341_SPI_Tx(uint8_t data)
{
	//fix! while(!__HAL_SPI_GET_FLAG(HSPI_INSTANCE, SPI_FLAG_TXE));
	while(!(SPI1->SR & SPI_SR_TXE)) {}
	Application::spiDma_Send(&data, 1);
	while (!(SPI1->SR & SPI_SR_TXE));
...
static void ILI9341_SPI_TxBuffer(uint8_t *buffer, uint16_t len)
{
	//fix! while(!__HAL_SPI_GET_FLAG(HSPI_INSTANCE, SPI_FLAG_TXE));
	while(!(SPI1->SR & SPI_SR_TXE)) {}
	Application::spiDma_Send(buffer, len);
	//HAL_SPI_Transmit(HSPI_INSTANCE, buffer, len, 10);
}

void ILI9341_WriteCommand(uint8_t cmd)
{
	LCD_DC_PORT->BRR =  (1 << LCD_DC_PIN);	//command
	//LCD_CS_PORT->BRR =  (1 << LCD_CS_PIN);	//select
	ILI9341_SPI_Tx(cmd);
...
{
	LCD_DC_PORT->BSRR =  (1 << LCD_DC_PIN);	//data
	//LCD_CS_PORT->BRR =  (1 << LCD_CS_PIN);	//select
	ILI9341_SPI_Tx(data);
	//HAL_GPIO_WritePin(LCD_CS_PORT, LCD_CS_PIN, GPIO_PIN_SET);	//deselect
}
...
{
	LCD_RST_PORT->BRR =  (1 << LCD_RST_PIN);	//Disable
	delayMs(10);
	LCD_CS_PORT->BRR =  (1 << LCD_CS_PIN);		//Select
	delayMs(10);
	LCD_RST_PORT->BSRR =  (1 << LCD_RST_PIN);		//Enable
	LCD_CS_PORT->BSRR =  (1 << LCD_CS_PIN); 		//Deselect
}
...
void ILI9341_DrawColorBurst(uint16_t color, uint32_t size)
{
	...
	if(SendingInBlock != 0)
	{
		...
		{
			//fix! HAL_SPI_Transmit(HSPI_INSTANCE, BurstBuffer, BufferSize, 10);
		}
	}

	//fix! HAL_SPI_Transmit(HSPI_INSTANCE, BurstBuffer, RemainderFromBlock, 10);
	LCD_CS_PORT->BSRR =  (1 << LCD_CS_PIN);
}
